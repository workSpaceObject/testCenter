...
export default {
  ...
  state: {
    ...
    eventImg:'testEvent.png',
    phtml:`<p style='padding-top: 164px;width: 50%; margin: 0 auto;font-size: 16px;text-indent: 31px;line-height: 30px;font-weight: bold;'>请返回登录 </p>`,
    deployBtn:false
  },
  ...
  effects: {
    ...
    *increLeaveCnt({payload},{call,put}){
     if(payload.type){
       ...
       if((parseInt(oldMaxLeaveCnt)-parseInt(leaveCnt))>0){
         ...
         yield put({type:'updateState',payload:{eventImg:'force.png'}})
       }
     }
    ...
    *saveStudentAns({payload},{call,put}){
      ...
      if(err){
        if(err.message=='forceDeployed'){
          yield put({type:'updateState',payload:{eventImg:'testEvent.png',phtml:`<p style='padding-top: 195px;width: 50%; margin-left: 176px;font-size: 42px;line-height: 30px;font-weight: bold;text-align: center;  color: #FF5500;'><span style='color:#666'>被强制交卷</span>`,btnIsShow:true}})
          yield put(routerRedux.replace({pathname:'/event'}));
        }
    ...
    *deployPaper({payload},{call,put}){
      const examineeUid=yield call(getLocalData,'examineeUid');
      yield put({type:'updateState',payload:{handVisible:false,eventImg:'deploying.png',isReply:false,phtml:'',deployBtn:false}})
      yield put(routerRedux.replace({pathname:'/event'}));
      //交卷前判断还有没有未提交成功的答案
      const newFail=yield call(getLocalData,'failansResults');
      ...
      if(data){
        ...
        yield put({type:'updateState',payload:{btnIsShow:true,eventImg:'testEvent.png',phtml:score!=null?`<p style='padding-top:185px;width: 50%; margin: 0 auto;font-size: 42px;line-height: 30px;font-weight: bold;text-align: center;  color: #FF5500;
'>${parseFloat(score)/10}<span style='color:#666'>分</span><br/><br/><span style='font-size: 25px;color:#666;'>考试结束</span></p>`:`<p style='padding-top:185px;width: 50%; margin: 0 auto;font-size: 42px;line-height: 30px;font-weight: bold;text-align: center;  color: #FF5500;
'><span style='font-size: 25px;color:#666;'>考试结束</span></p>`}})
      }else if(err){
        if(err&&err.message=='forceDeployed'){
          yield put({type:'updateState',payload:{eventImg:'testEvent.png',phtml:`<p style='padding-top: 195px;width: 50%; margin-left: 176px;font-size: 42px;line-height: 30px;font-weight: bold;text-align: center;  color: #FF5500;'><span style='color:#666'>被强制交卷</span>`,btnIsShow:true}})
        }else {
          yield put({type:'updateState',payload:{eventImg:'testEvent.png',phtml:`<p style='padding-top: 195px; margin-left: 176px;font-size: 42px;line-height: 30px;font-weight: bold;text-align: center;  color: #FF5500;'><span style='color:#666'>交卷失败</span><br/><br/><span style="color:red;font-size: 21px;">${err.message}</span></p>`,deployBtn:true}})
        }
      }
    },
    ...
    *getReplyedHelpEvent({payload},{call,put}){
      ...
      if(data){
        if(data.data){
          const datas=data.data;
          yield put(routerRedux.replace({pathname:'/event'}));
          yield put({type:'updateState',payload:{btnIsShow:true,eventImg:'helped.png',phtml:`<p style='padding-top: 164px;width: 50%; margin: 0 auto;font-size: 16px;text-indent: 31px;line-height: 30px;font-weight: bold;'>${datas.replyContent}</p>`}})
        }
      }
    },

    //导出本地答案
    *downloadAnswer({payload},{call,put}){
      const newexaminee=yield call(getLocalData,'examinee');
      let examinee=JSON.parse(newexaminee)
      if(examinee&&payload==examinee.identifier){
        const newFail=yield call(getLocalData,'failansResults');
        const failansResults=JSON.parse(newFail);
        if(failansResults){
          if(failansResults[examinee.uid]){
            //导出文件
            var blob = new Blob([escape(JSON.stringify(failansResults[examinee.uid]))], {type: "text/plain;charset=utf-8"});
            FileSaver.saveAs(blob, "答案.txt");
          }else {
            notification.error({message:'没有可以导出的答案'})
          }
        }else {
          notification.error({message:'没有可以导出的答案'})
        }
